...
The Analyzer app is mid-redesign per `docs/ANALYZER_REDESIGN_PLAN.md`, audited and revised 2026-09-25 (splitting the 6,678-line `App.jsx` into routed pages, real Tailwind, shared design tokens, deep-linkable Character/Team/Match pages, share-snippet export). **Phase 1 (Foundation)** is done: `react-router-dom` is wired in `src/main.jsx` behind a catch-all route, `src/routes.js` defines the target URL scheme (though nothing imports it yet), and the aggregation math has been fully extracted into `src/utils/aggregation/`. **Phase 1.5 (Data layer)** is done: a build-time compact match corpus (`apps/analyzer/scripts/generate-br-aggregates.js` → `public/br-aggregates/`, wired into the analyzer prebuild) replaced the old ~2,232-file / ~67 MB page load with 2 requests / ~101 KB gzipped, verified against the raw data by `npm run verify-aggregates`. **Phase 2a is done** too: real Tailwind v3 now runs in the analyzer alongside the legacy App.css (load order is load-bearing — see that app's CLAUDE.md), and shared design tokens live in `packages/ui/src/tokens.js`, consumed by all four Tailwind configs with each app's legacy token names kept as aliases. Canonical accent is `#f97316`. **Phase 2b (App.css teardown) is next.** One cross-cutting blocker remains: the deploy workflow's `cp dist/index.html dist/404.html` means **analyzer deep links 404 into the website SPA**, so a smart 404 dispatcher is a prerequisite of Phase 3. Check that doc's "Progress" section before starting analyzer redesign work, and keep it updated as phases complete.
