...
|-----|------|-------|---------|---------|
| Website | `apps/website/` | `/` | Public site — home, teams, standings, events (season-scoped off-shoot competitions), rules, community, archives. React Router SPA, content driven by YAML files in `public/content/`. Also serves the `/cms` Decap CMS admin panel and the standalone `/schedule-import` tool. | `apps/website/CLAUDE.md` |
